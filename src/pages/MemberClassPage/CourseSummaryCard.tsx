...
const messages = defineMessages({
  courseSummary: { id: 'memberClass.card.courseSummary', defaultMessage: '課程摘要' },
  ...
  expiryDate: { id: 'memberClass.card.expiryDate', defaultMessage: '到期日' },
})
...
interface CourseSummaryCardProps {
  summaries: CoursePackageSummary[]
}

const CourseSummaryCard: React.FC<CourseSummaryCardProps> = ({ summaries }) => {
  const { formatMessage } = useIntl()
  const [collapsedSections, setCollapsedSections] = useState<Record<string, boolean>>({})

  const groupedSummaries = useMemo(() => {
    return summaries.reduce<Record<string, CoursePackageSummary[]>>((acc, summary) => {
  ...
  return (
    <StyledCard>
      <StyledTitle>{formatMessage(messages.courseSummary)}</StyledTitle>
          ...
          {!collapsedSections[language] && (
            <StyledSectionContent>
              {groupedSummaries[language].map(summary => {
                const remaining = summary.totalLessons - summary.completedLessons
                return (
                  <StyledCourseItem key={summary.id}>
                    ...
                    <StyledCourseInfo>
                      <span>{formatMessage(messages.expiryDate)}：</span> {summary.expiryDate}
                    </StyledCourseInfo>
                  </StyledCourseItem>
                )
              })}
