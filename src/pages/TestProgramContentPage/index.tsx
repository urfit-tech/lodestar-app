...
const ProgramContentPage: React.VFC = () => {
  ...
  const { program, loadingProgram, refetchProgram } = useProgram(programId)
  ...
  const handlePrevContent = () => {
    const prevProgramContent = flatten(program?.contentSections.map(v => v.contents)).find(
      (_, i, contents) => contents[i + 1]?.id === currentProgramContentId,
    )
    setCurrentProgramContentId(prevProgramContent?.id || '')
    window.history.pushState('', prevProgramContent?.title || '', `${prevProgramContent?.id}?back=${previousPage}`)
  }
  ...
  const handleNextContent = () => {
    ...
    window.history.pushState('', nextProgramContent?.title || '', `${nextProgramContent?.id}?back=${previousPage}`)
  }

  return (
    <Layout>
      {!loadingApp && <ProgramContentPageHelmet program={program!} contentId={currentProgramContentId} />}
      <StyledPageHeader
                ...
                <div className={menuVisible ? 'd-lg-block col-lg-9 d-none' : 'col-12'}>
                  <StyledLayoutContent>
                    <ProgramContentBlock
                      ...
                      programContentSections={program.contentSections}
                      programContentId={currentProgramContentId}
