...
import { getFileDownloadableLink, hasJsonStructure } from '../../helpers'
...
const ProgramContentPage: React.VFC = () => {
  ...
  const [currentProgramContentListIdx, setCurrentProgramContentListIdx] = useState(0)
  const programContentList = useMemo(() => {
    return program?.contentSections.flatMap(contentSection => contentSection.contents.map(content => content)) || []
  }, [program?.contentSections])

  useEffect(() => {
    if (programContentList[currentProgramContentListIdx]?.contentType === 'audio') {
      getFileDownloadableLink(
        `audios/${appId}/${programId}/${programContentList[currentProgramContentListIdx]?.id}`,
        authToken,
      ).then(url => {
        setAudioUrl(url)
      })
    }
  }, [appId, authToken, currentProgramContentListIdx, programContentId, programContentList, programId])
  ...
  return (
    <Layout>
      {!loadingApp && (
        <ProgramContentPageHelmet program={program!} contentId={programContentList[currentProgramContentListIdx].id} />
      )}
      <StyledPageHeader
      ...
      <StyledLayoutContent>
                ...
                <div className={menuVisible ? 'd-lg-block col-lg-9 d-none' : 'col-12'}>
                  <StyledLayoutContent>
                    <ProgramContentBlock
                      ...
                      programContentSections={program.contentSections}
                      programContentId={programContentList[currentProgramContentListIdx].id}
        ...
        {programContentList[currentProgramContentListIdx]?.contentType === 'audio' && (
          <AudioPlayer
            autoPlay
            title={programContentList[currentProgramContentListIdx]?.title}
            audioUrl={audioUrl}
            ...
            onNext={() => {
              window.history.pushState(
                '',
                programContentList[currentProgramContentListIdx + 1].title,
                `${programContentList[currentProgramContentListIdx + 1].id}?back=${previousPage}`,
              )
              setCurrentProgramContentListIdx(currentProgramContentListIdx + 1)
            }}
          />
        )}
      </StyledLayoutContent>
