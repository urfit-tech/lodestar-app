...
const ProgramContentBlock: React.VFC<{
  ...
  const { loadingProgramContent, programContent } = useProgramContent(currentProgramContentId)
  const { hasProgramContentPermission, isLoginTrial } = useHasProgramContentPermission(programContentId)
  const [audioUrl, setAudioUrl] = useState<string>()
  const [prevProgramContent, setPrevProgramContent] = useState(
    flatten(programContentSections.map(v => v.contents)).find(
      (_, i, contents) => contents[i + 1]?.id === programContentId,
    ),
  )
  const [nextProgramContent, setNextProgramContent] = useState(
    flatten(programContentSections.map(v => v.contents)).find(
      (_, i, contents) => contents[i - 1]?.id === programContentId,
    ),
  )
  ...
  useEffect(() => {
    if (!audioUrl && programContentBodyType === 'audio') {
      getFileDownloadableLink(`audios/${appId}/${programId}/${currentContentId}`, authToken).then(url => {
        setAudioUrl(url)
      })
    }
  }, [programContentBodyType, currentContentId, programId])
        ...
        ((hasProgramContentPermission && moment().isAfter(moment(programContent.publishedAt))) ||
          ...
          <AudioPlayer
            ...
            onPrev={
              prevProgramContent
                ? () => {
                    setPrevProgramContent(
                      flatten(programContentSections.map(v => v.contents)).find(
                        (_, i, contents) => contents[i + 1]?.id === prevProgramContent.id,
                      ),
                    )
                    setNextProgramContent(
                      flatten(programContentSections.map(v => v.contents)).find(
                        (_, i, contents) => contents[i - 1]?.id === prevProgramContent.id,
                      ),
                    )
                    setCurrentProgramContentId(prevProgramContent.id)
                    getFileDownloadableLink(`audios/${appId}/${programId}/${prevProgramContent.id}`, authToken).then(
                      url => {
                        setAudioUrl(url)
                      },
                    )
            ...
            onNext={
              nextProgramContent
                ? () => {
                    setPrevProgramContent(
                      flatten(programContentSections.map(v => v.contents)).find(
                        (_, i, contents) => contents[i + 1]?.id === nextProgramContent.id,
                      ),
                    )
                    setNextProgramContent(
                      flatten(programContentSections.map(v => v.contents)).find(
                    ...
                    setCurrentProgramContentId(nextProgramContent.id)
                    getFileDownloadableLink(`audios/${appId}/${programId}/${nextProgramContent.id}`, authToken).then(
                      url => {
                        setAudioUrl(url)
                      },
                    )
