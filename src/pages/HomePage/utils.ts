...
const useLastWatchedProgramContents = () => {
  const { data } = useQuery<hasura.GET_LAST_WATCHED_PROGRAM_CONTENTS>(
    gql`
  ...
  return (
    data?.program_content_progress.map(programContentProgress => ({
      ...
      role: {
        ...
        pictureUrl:
          programContentProgress.program_content.program_content_section.program.program_roles[0].member?.picture_url ||
          '',
      },
    })) || []
  )
}
const useLatestPrograms: () => SunkProgramProps[] = () => {
  const { data } = useQuery<hasura.GET_LAST_PROGRAMS>(
    gql`
...
const useAffordablePrograms: () => SunkProgramProps[] = () => {
  const { data } = useQuery<hasura.GET_AFFORDABLE_PROGRAMS>(
    gql`
      ${programFieldsFragment}
...
const useHottestTagPrograms: () => SunkProgramProps[] = () => {
  const { data } = useQuery<hasura.GET_HOTTEST_TAG_PROGRAMS>(
    gql`
      ${programFieldsFragment}
...
const useUnitCategoryPrograms: () => SunkProgramProps[] = () => {
  const { data } = useQuery<hasura.GET_UNIT_CATEGORY_PROGRAMS>(
    gql`
      ${programFieldsFragment}
...
const useSystematicCategoryPrograms: () => SunkProgramProps[] = () => {
  const { data } = useQuery<hasura.GET_SYSTEMATIC_CATEGORY_PROGRAMS>(
    gql`
      ${programFieldsFragment}
...
const useSunkHomePageProducts: () => {
  ...
  lastWatchedProgramContents: useLastWatchedProgramContents(),
  latestPrograms: useLatestPrograms(),
  affordablePrograms: useAffordablePrograms(),
  hottestTagPrograms: useHottestTagPrograms(),
  unitCategoryPrograms: useUnitCategoryPrograms(),
  systematicCategoryPrograms: useSystematicCategoryPrograms(),
})
